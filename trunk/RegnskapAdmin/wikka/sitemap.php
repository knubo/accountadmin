...
';
$pages = mysql_query("SELECT SQL_NO_CACHE tag, time,(select match(body) against ('CategorySitemap' in boolean mode) from wikka_pages W where P.id=W.id) as sitecat FROM " . $wakkaConfig['table_prefix'] . "pages P LEFT JOIN " .
        $wakkaConfig['table_prefix'] . "acls ON page_tag = tag WHERE latest = 'Y' AND (read_acl = '*' OR read_acl IS NULL)");
