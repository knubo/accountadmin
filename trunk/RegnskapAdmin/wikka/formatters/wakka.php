...
if (!function_exists("wakka2callback")) # DotMG [many lines] : Unclosed tags fix!
{
	function wakka2callback($things)
	{
		...
		else if (preg_match("/:::(.+?):::/s", $thing, $matches))
		{
			...
			"\[\[[^\[]*?\]\]|".																	# forced link
			"\b[a-z]+:\/\/\S+|".																	# URL
			"\b[A-Z≈ƒ÷‹][A-Za-z≈ƒ÷‹ﬂÂ‰ˆ¸]+[:](?![=_])\S*\b|".										# InterWiki link
			"\b([A-Z≈ƒ÷‹]+[a-zﬂÂ‰ˆ¸]+[A-Z0-9≈ƒ÷‹][A-Za-z0-9≈ƒ÷‹ﬂÂ‰ˆ¸]*)\b|".						# CamelWords
        ...
        function parse_attributes($attribs, $hints) {
                ...
                foreach ( $attribs as $attrib )
                {
                        ...
                        if (!$a)
                        {
                                ...
                                // When migrating to UTF-8, we could use str_replace('--', '‚<88><92>‚<88><92>', $key) to make things more pretty. //TODO garbled ... mdash?
